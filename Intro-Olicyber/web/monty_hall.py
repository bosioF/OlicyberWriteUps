...
def make_requests(session, choice):
    r = requests.post(url,cookies={'session':f'{session}'},data={'choice': f'{choice}'}, allow_redirects=False)
    return r.text, r.cookies['session']

def solve():
    _, current_session = make_requests('asd','1')

    for _ in range(12):
        new_sessions = []
        for i in range(1, 4):
            content, new_session = make_requests(current_session, i)
